...
std::vector<PosePoint> RTMPoseOnnxruntime::Inference(const cv::Mat& input_mat, const DetectBox& box)
{
	...
	std::vector<Ort::Float16_t> input_image_array;
	input_image_array.resize(1 * image_channels * image_height * image_width);

	Ort::Float16_t* input_image = input_image_array.data();
	...
	std::vector<const char*> m_onnx_input_names{ "input" };
	std::vector<const char*> m_onnx_output_names{ "output","686" };
	...
	for (int i = 0; i < joint_num; ++i)
	{
		...
		float score = ((score_x) > (score_y)) ? (score_x) : (score_y);
...
std::pair<cv::Mat, cv::Mat> RTMPoseOnnxruntime::CropImageByDetectBox(const cv::Mat& input_image, const DetectBox& box)
{
	...
	float aspect_ratio = 256.0 / 256.0;
	...
	float scale_image_width = box_width * 1.4;
	float scale_image_height = box_height * 1.4;
	...
	cv::warpAffine(input_mat_copy, affine_image, affine_transform, cv::Size(256, 256), cv::INTER_LINEAR);
...
void RTMPoseOnnxruntime::PrintModelInfo(Ort::Session& session)
{
	...
	for (auto i = 0; i < num_input_nodes; i++)
		std::cout << "The input op-name " << i << " is:" << session.GetInputName(i, allocator) << std::endl;
	for (auto i = 0; i < num_output_nodes; i++)
		std::cout << "The output op-name " << i << " is:" << session.GetOutputName(i, allocator) << std::endl;
